...
if opt.outfile == 'peak_overlap_between_ablife_and_piranha_cims.txt':
...
def main():
    ...
    for eachLine in open(opt.ctrl,'r'):
        ...
        if eachLine.startswith("#"):
            continue
        peak_iv = HTSeq.GenomicInterval(line[0], int(line[1]), int(line[2]), line[5])
        gas[peak_iv]+="piranha:"+eachLine.strip()
        piranha_total_peaknum += 1

    title3 = getTitle(opt.cims)
    cims_total_peaknum = 0
    for eachLine in open(opt.cims,'r'):
        line=eachLine.strip().split("\t")
        if eachLine.startswith("#"):
            continue
        peak_iv = HTSeq.GenomicInterval(line[0], int(line[1]), int(line[2]), line[5])
        gas[peak_iv]+="cims:"+eachLine.strip()
        cims_total_peaknum += 1
    ...
    w.writelines("ablife"+title1+"\tpiranha"+title2+"\tcims"+title3+"\n")
    ...
    for eachLine in open(opt.exp,'r'):
        ...
        for iv, fs in gas[peak_iv].steps():
            if len(fs)>1:
                t = set()
                for p in fs:
                    if p.startswith("piranha:") :
                        t.add("piranha")
                    if p.startswith("cims:") :
                        t.add("cims")
                if len(t) == 2:
                    # print(t)
                    # print(fs)
                    flag = 1
                    w.writelines(eachLine.strip()+"\t")
                    for p in fs:
                        if p.startswith("piranha:") :
                            p = re.sub('piranha:','',p)
                            w.writelines(p+"\t")
    ...
    tmp = os.popen('cut -f 21 '+opt.outfile+' | sort|uniq|wc -l').readlines()
    cims_peaknum = int(tmp[0].strip()) - 1
    ...
    cims_overlap_percent = round(100 * float(cims_peaknum) / cims_total_peaknum, 2)
    ...
    print("cims total peaks:"+str(cims_total_peaknum))
    print("cims overlap peaks:"+str(cims_peaknum)+ "(" + str(cims_overlap_percent) + "%)" )
