...
def configOpt():
    ...
    p.add_option('-i', '--info', dest='info', action='store', type='string', help='peak info')
    p.add_option('-e', '--extend', dest='extend', default=70, action='store', type=int)
    p.add_option('-o', '--outfile', dest='outfile', default='trans_length.txt', action='store', type='string', help='trans_length.txt')
...
def main():
    ...
    for gene in db.features_of_type(genes):
        gene_id = gene.id
        lens[gene_id]=gene.end - gene.start + 1
        chr = gene.seqid
        ...
        for isoform in db.children(gene_id, level=1, featuretype=trans):
            for gu in db.children(isoform.id, level=1, featuretype='exon'):
                gu_start = gu.start
                gu_end = gu.end
                gu_iv = HTSeq.GenomicInterval(chr, gu_start - 1, gu_end, ".")
                ga[gu_iv] = 1
            break
    ...
    for line in open(opt.info):
        ...
        if not line[6].startswith('intron'): continue
        if str(line[4]) < 2 : continue
        gene_id = line[7]
        ext_iv = HTSeq.GenomicInterval(line[0], int(line[1]) - opt.extend, int(line[2]) + opt.extend, ".")
        base[gene_id][ext_iv] = 1

    with open(opt.outfile, 'w') as o:
        for gene in base:
            tran_len = 0
            for iv, value in base[gene].steps():
                if value >= 1:
                    iv_len = iv.length
                    tran_len += iv_len
            if tran_len==0:
                tran_len = lens[gene]
            o.writelines(gene + '\t' + str(tran_len) + '\n')
...
def countProgram(programName, startT, runT, isTest):
    ...
    countProgramFile.write(programName + '\t' + str(os.getlogin()) + '\t' + str(startT) + '\t' + str(runT) + 's\t' + isTest + '\n')
