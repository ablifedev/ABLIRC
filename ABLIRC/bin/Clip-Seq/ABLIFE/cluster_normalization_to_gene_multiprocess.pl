...
my %opts;
GetOptions(\%opts,"gff=s","peak=s","strand=s","o=s","h" );

if(!defined($opts{gff}) || !defined($opts{peak}) || !defined($opts{o})|| defined($opts{h})   )
{
	print <<"	Usage End.";
	...
	Usage:perl $0
		...
		-strand        strand option   sense|antisense|unstrand,default is sense
...
my $gff=$opts{gff};
my $peak=$opts{peak};
my $strandoption=$opts{strand} || "sense";
my $out=$opts{o};

if($strandoption ne "sense" && $strandoption ne "antisense" && $strandoption ne "unstrand"){
	die "-strand must be one of sense|antisense|unstrand";
}
...
foreach my $chr (sort keys %gene){
    ...
    if ($pid == 0) {
        ...
        push @pids, $pid;
        my %usedpeaks = ();
        open (TMPOUT,">$out\_$chr") || die "Can't creat $out\_$chr\n";		#open a file as writing
        foreach my $thisgene (@{$gene{$chr}}){
			...
			foreach my $thispeak (@{$peaks{$chr}}){
				...
				if($strandoption eq "sense"){
					if($genestrand ne $this_peak_stand){
						next ;
					}
				}elsif($strandoption eq "antisense"){
					if($genestrand eq $this_peak_stand){
						next ;
					}
				}
				...
				# print $chr,"\t",$this_peak_start,"\t",$this_peak_end,"\n";
			...
			if (@gene_peak) {
					...
					if ($current_peak_num/$current_tag_num<1) {
						...
						print TMPOUT "Gene:\t$genestart\t$geneend\t$genename\t$genestrand\n";
						for (my $i=0;$i<=$#gene_peak ;$i++) {
							print TMPOUT $gene_peak[$i];
						}
						print TMPOUT $current_sum_tag_len,"\t",$current_len_gene,"\t",$current_peak_num,"\t",$current_tag_num,"\t",$average_nucleotide_distribution,"\n";
					}
