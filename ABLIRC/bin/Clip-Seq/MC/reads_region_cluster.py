...
def configOpt():
    ...
    p.add_option('-m', '--mindepth', dest='mindepth', action='store', type='int', help='mindepth', default=3)
...
def reads_cluster(bedFile, fout, mindepth, db):
    ...
    for gene in db.features_of_type('gene'):
        ...
        for iv, value in depth_ga[gene_iv].steps():
            if value >= mindepth:
                m += 1
                id = iv.chrom + "_" + str(m)

                fout.writelines(">" + id + "\t" + iv.chrom + "\t" + str(iv.start) + "\t" + str(iv.end) + "\t" + str(value) + "\t" + iv.strand + "\n")
...
def main():
    ...
    fout.writelines("#>id_in_chr\tchr\tstart\tend\ttags\tmaxheight\tsummit\tstrand\tchr\n")
    ...
    reads_cluster(opt.bed, fout, opt.mindepth, db)
