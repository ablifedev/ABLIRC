...
def configOpt():
    ...
    p.add_option(
        '-n', '--norm', dest='norm', default=False, action='store_true',
        help='nomalize with TPM')
...
def main():
    ...
    TMR = getBamReadsNumber(opt.bamfile)

    # norm = opt.norm

    peakDistributionToSummit(opt.bamfile, opt.peakfile, opt.outfile, opt.halfwinwidth, type=opt.peaktype)

    cmd = 'Rscript ' + scriptPath + '/../plot/Line_single_ggplot2.r -f ' + opt.outfile + ' -t "peakreads distribution to summit" -n peakreads_distribution_to_summit -o ./'
    os.system(cmd)
