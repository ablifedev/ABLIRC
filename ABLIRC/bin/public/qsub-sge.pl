...
=head1 Usage
  ...
  --queue <str>     specify the queue to use, default general.q
  --usesge <yes/no>    use sge to run the jobs, default yes
...
my ($Queue, $Interval, $Lines, $Maxjob, $Maxproc, $Usesge, $Convert,$Secure,$Reqsub,$Resource,$Verbose, $Help);
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"maxproc:i"=>\$Maxproc,
...
while (@Shell) {
	...
	for (my $i=0; $i<@Shell; $i++)
	{
		...
		while (1)
		{
			...
			#print $jobn,"\n";
			if ($jobn < $Maxproc) {
				my $job_return_cmd = "$job_cmd $Shell[$i]";
				if ($Usesge eq "no"){
					$job_return_cmd = "$job_cmd $Shell[$i] 1>$Shell[$i]\.o$j 2>$Shell[$i]\.e$j &";
				}
				my $job_return = `$job_return_cmd`;
				my $job_id = $j;
				$job_id = $1 if($job_return =~ /Your job (\d+)/);
	...
	while (1) {
		...
		if ($Usesge eq "no"){
			...
			$run_num = run_count(\%Alljob,\%Runjob);
		}
		last if($run_num == 0);
	...
	$qsub_cycle++;
	if($qsub_cycle > 3){
		print OUT "\n\nProgram stopped because the reqsub cycle number has reached 3, the following jobs unfinished:\n";
		foreach my $job_id (sort keys %Error) {
			my $shell_file = $Error{$job_id};
			print OUT $shell_file."\n";
		}
		print OUT "Please check carefully for what errors happen, and redo the work, good luck!";
		die "\nProgram stopped because the reqsub cycle number has reached 3\n";
	}
