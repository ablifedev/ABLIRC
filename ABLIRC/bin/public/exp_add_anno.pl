...
my %opts;
GetOptions(\%opts,"exp=s","anno=s","column=s","force=s","annonum=s","rc=s","r","c","u","o=s","h" );
...
while(<EXP>){
	...
	my @line=split(/\t/);
	# Gene 	 Chromosome 	logConc	logFC	p.value	Tair_Gene
	if(/^Gene\s+/i or /^X.ID/i or /^miRNA\s+/i or /^\#/ or /^X\./ or /^\+/ or /^CircRNA\s+/ or /^consensus.cluster/ or /^Chr\s+/i){
		print OUT $_,"\t",$title,"\n";
		next;
	}
