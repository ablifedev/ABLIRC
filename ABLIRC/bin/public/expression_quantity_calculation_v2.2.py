...
def configOpt():
    ...
    p.add_option('-b', '--bam', dest='bam', action='store', type='string', help='bam file')
    p.add_option('-o', '--outfile', dest='outfile', default='expression_profile.txt', action='store', type='string', help='gene expression file')
...
def readChr(chr, reads, check, TMR):
    ...
    for gene in db.features_of_type('gene', seqid=chr, order_by='start'):
        ...
        for isoform in db.children(gene_id, level=1, featuretype=trans):
            ...
            for r in bamfile[isoform_iv]:
                ...
                if r.paired_end:
                    ...
                    if usedreads.has_key(r_name):
                        if usedreads[r_name] == "":
                            ...
                            anti_info = usedreads[r_name].split(":")
                    ...
                    if usedreads.has_key(r_name):
                        if usedreads[r_name] == "":
                            continue
                        elif ((r.iv.strand == isoform_iv.strand and r.pe_which == "first") or (r.iv.strand != isoform_iv.strand and r.pe_which == "second")):
                            anti_info = usedreads[r_name].split(":")
                            reads_dict[anti_info[0]]["exon"]["anti"] -= float(anti_info[1])
                        else:
                            continue
                ...
                if r.paired_end:
                    if r.mate_aligned:
                        m_start = r.iv.start
                        m_end = r.mate_start.pos
                        insert_size = r.inferred_insert_size
                        if insert_size > 0:
                            r_len = insert_size - (m_end - m_start) + r_len
                        else:
                            r_len = 0 - insert_size - (m_start - m_end) + r_len

                # print(r_name+"\t"+str(r.inferred_insert_size)+"\t"+str(r.mate_start)+"\t"+str(r.mate_aligned))
                if ((r.iv.strand == isoform_iv.strand and (not r.paired_end)) or (r.paired_end and r.iv.strand == isoform_iv.strand and r.pe_which == "first") or (r.paired_end and r.iv.strand != isoform_iv.strand and r.pe_which == "second")):
                    ...
                    if (r.paired_end and r.iv.strand != isoform_iv.strand and r.pe_which == "second"):
                    ...
                    for iv in iv_seq:
                        ...
                        for iv2, value2 in ga[iv].steps():
                            ...
                            usedreads[r_name] = ""
                elif ((r.iv.strand != isoform_iv.strand and (not r.paired_end)) or (r.paired_end and r.iv.strand != isoform_iv.strand and r.pe_which == "first") or (r.paired_end and r.iv.strand == isoform_iv.strand and r.pe_which == "second")):
                    iv_seq_reverse = (invert_strand(co.ref_iv) for co in r.cigar if co.type == "M" and co.size > 0)
                    if (r.paired_end and r.iv.strand == isoform_iv.strand and r.pe_which == "second"):
                        iv_seq_reverse = (co.ref_iv for co in r.cigar if co.type == "M" and co.size > 0)
                    reverse_len = 0
                    reverse_flag = 0
                    for iv in iv_seq_reverse:
                        for iv2, value2 in ga[iv].steps():
                            ...
                            reverse_flag = 1
                    if reverse_flag == 1:
                        usedreads[r_name] = gene_id + ":" + str(reverse_len)


        reads_dict[gene_id]["exon"]["depth"] = round(float(exon_total_base) / exon_len, 2)
        ...
        reads_dict[gene_id]["exon"]["coverage"] = round(float(coverage_len) / exon_len, 2)

        reads_dict[gene_id]["exon"]["rpkm"] = round(1e9 * int(reads_dict[gene_id]["exon"]["sense"] + 0.5) / exon_len / TMR, 2)
                        ...
                        for iv2, value2 in coverage[iv].steps():
                            if value2 > 0:
                                for j in range(iv2.start, iv2.end):
                                    temp_randCheck_mRNA_list[int(100 * (sum_exon_len + j - iv.start) / exon_len)] += value2
                ...
                for iv, value in ga[gene_iv].steps():
                    if value > 0:
                        for iv2, value2 in coverage[iv].steps():
                            if value2 > 0:
                                for j in range(iv2.start, iv2.end):
                                    temp_randCheck_mRNA_list[int(100 * (exon_len - sum_exon_len - j + iv.start - 1) / exon_len)] += value2
                        sum_exon_len += iv.length
                for x in range(100):
                    check_dict["randCheck_mRNA"][x] += temp_randCheck_mRNA_list[x] / window
...
def readChr_unstrand(chr, reads, check, TMR):
    print(chr)
    reads_dict = {}
    check_dict = {}
    check_dict["randCheck_gene"] = [0 for x in range(100)]
    ...
    for gene in db.features_of_type('gene', seqid=chr, order_by='start'):
        ...
        ga = HTSeq.GenomicArray([chr], stranded=False, typecode='i')
        ...
        for exon in db.children(gene_id, featuretype="exon"):
            ...
            exon_start = exon.start
            exon_end = exon.end
            exon_iv = HTSeq.GenomicInterval(chr, exon_start - 1, exon_end, ".")
            if exon_iv.length == 0:
                continue
            ga[exon_iv] = 1


        gene_iv = HTSeq.GenomicInterval(chr, gene_start - 1, gene_end, ".")
        gene_len = gene_iv.length
        exon_len = 0
        for iv, value in ga[gene_iv].steps():
            if value >= 1:
        ...
        exon_total_base = 0
        coverage = HTSeq.GenomicArray([chr], stranded=False, typecode="i")
...
def main():
    ...
    for chr in db.seqids():
        ...
        check[chr] = {}
        if opt.unstrand:
            pool.apply_async(readChr_unstrand, args=(chr, reads, check, TMR))
        else:
    ...
    fout.writelines("#Gene\tChro\tStrand\tStart\tEnd\tLength\tExonLen\tIntroLen\tDepth\tCoverage\tRPKM\tTotalReads\tSenseReads\tAntisenseReads\tGeneType\n")
    ...
    for chr in sorted(d.keys()):
        ...
        for gene in sorted(dict(d[chr]).keys()):
            fout.writelines(d[chr][gene]["exon"]["gene_name"] + "\t" + chr + "\t" + str(d[chr][gene]["exon"]["gene_strand"]) + "\t" + str(d[chr][gene]["exon"]["gene_start"]) + "\t" + str(d[chr][gene]["exon"]["gene_end"]) + "\t" + str(d[chr][gene]["exon"]["gene_len"]) + "\t" + str(d[chr][gene]["exon"]["exon_len"]) + "\t" + str(d[chr][gene]["exon"]["gene_len"] - d[chr][gene]["exon"]["exon_len"]) + "\t" + str(d[chr][gene]["exon"]["depth"]) + "\t" + str(d[chr][gene]["exon"]["coverage"]) + "\t" + str(d[chr][gene]["exon"]["rpkm"]) + "\t" + str(int(d[chr][gene]["exon"]["sense"] + 0.5) + int(d[chr][gene]["exon"]["anti"] + 0.5)) + "\t" + str(int(d[chr][gene]["exon"]["sense"] + 0.5)) + "\t" + str(int(d[chr][gene]["exon"]["anti"] + 0.5)) + "\t" + d[chr][gene]["exon"]["gene_type"] + "\n")
