...
def configOpt():
    ...
    p.add_option(
        '-o', '--outfile', dest='outfile', default="aroundtss.txt", action='store',
        type='string', help='aroundtss.txt')
    p.add_option(
...
if opt.outfile == 'aroundtss.txt':
    opt.outfile = sample + 'aroundtss.txt'

stat_file = sample + 'aroundtss.txt'
...
binPath = "/".join(scriptPath.split("/")[0:-3])  # absolute bin path
...
def main():
    ...
    for chr in db.seqids():
        pool.apply_async(iv_distribution_around_gene_bychr,
                         args=(chr, opt.tc, opt.db, "tss", opt.halfwinwidth))
    pool.close()
    pool.join()
    ...
    with open(opt.outfile, 'w') as o:
        o.writelines(
            "#chr\tisoform_s\tisoform_e\tstrand\tisoform_id\tgene_id\ttc_s\ttc_e\ttc_id\ttc_tpm\ttc_len\tdistance2tss\n")
