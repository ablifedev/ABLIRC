...
my %opts;
GetOptions(\%opts,"exp=s","t=s","symbol=s","o=s","h" );
...
if($symbol==0){
	foreach my $sample (sort keys %sample_hash){
		...
		$num = "\$".$num;
		$rpkm{$sample}{"0"}=`more $exp_file | awk '$num>0' | awk '{print \$1}' | sort | uniq | wc -l`;
		chomp($rpkm{$sample}{"0"});
		$rpkm{$sample}{"0"}-=1;
		$rpkm{$sample}{"1"}=`more $exp_file | awk '$num>=1' | awk '{print \$1}' | sort | uniq | wc -l`;
		chomp($rpkm{$sample}{"1"});
		$rpkm{$sample}{"1"}-=1;
...
if($symbol!=0){
	foreach my $sample (sort keys %sample_hash){
		...
		# print $sym_num,"\n";
		$sym_num = "\$".$sym_num;
		$rpkm{$sample}{"0"}=`more $exp_file | awk '$num>0' | awk '{print $sym_num}' | sort | uniq | wc -l`;
		chomp($rpkm{$sample}{"0"});
		$rpkm{$sample}{"0"}-=1;
		$rpkm{$sample}{"1"}=`more $exp_file | awk '$num>=1' | awk '{print $sym_num}' | sort | uniq | wc -l`;
		chomp($rpkm{$sample}{"1"});
		$rpkm{$sample}{"1"}-=1;
		print $rpkm{$sample}{"0"},"\n";
		# print OUT $sample,"\t",$total_count,"\t",$rpkm{$sample}{"0"},"(",sprintf("%3.2f%%",$rpkm{$sample}{"0"}/$total_count*100),")\t",$rpkm{$sample}{"1"},"(",sprintf("%3.2f%%",$rpkm{$sample}{"1"}/$rpkm{$sample}{"0"}*100),")\n";
