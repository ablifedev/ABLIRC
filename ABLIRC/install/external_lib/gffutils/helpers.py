...
def make_query(args, other=None, limit=None, strand=None, featuretype=None,
    ...
    _QUERY = ("{_SELECT} {OTHER} {EXTRA} {FEATURETYPE} {SEQID} "
    ...
    d = dict(_SELECT=constants._SELECT, OTHER="", FEATURETYPE="", SEQID="", LIMIT="",
    ...
    if seqid:
        # Handle single or iterables of seqid.
        #
        # e.g., "seqid = 'exon'"
        #
        # or, "seqid IN ('chr1', 'chr2')"
        # print(seqid)
        if isinstance(seqid, six.string_types):
            d['SEQID'] = "features.seqid = ?"
            args.append(seqid)
        else:
            d['SEQID'] = (
                "features.seqid IN  (%s)"
    ...
    if "where" in d['OTHER'].lower():
        where = True
    for i in ['EXTRA', 'FEATURETYPE', 'LIMIT', 'STRAND', 'SEQID']:
        if d[i]:
            if not where:
                d[i] = "WHERE " + d[i]
                where = True
